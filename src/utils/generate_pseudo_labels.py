...
for img_name in image_files:
    ...
    if results.boxes is not None and len(results.boxes) > 0:
        for mask, box, conf, cls in zip(results.masks.xyn, results.boxes, results.boxes.conf, results.boxes.cls):
            
            w, h = box.xywhn[0][2].item(), box.xywhn[0][3].item()
            area = w * h 
            
            # Jeśli maska ma wystarczającą pewność (>=0.45) i nie jest całym niebem (<=0.35) -> Bierzemy!
            if float(conf) >= CONF_THRESHOLD and area <= MAX_AREA_THRESHOLD:
                flat_coords = [str(c) for point in mask for c in point]
                line = f"{int(cls)} " + " ".join(flat_coords)
