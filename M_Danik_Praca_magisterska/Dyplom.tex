...
% Define custom colors for Python
\definecolor{keywordcolor}{rgb}{0.0, 0.0, 0.8}    % Dark Blue
%\definecolor{stringcolor}{rgb}{0.58, 0.0, 0.82}   % Purple
\definecolor{stringcolor}{rgb}{0.85, 0.4, 0.0}
\definecolor{ndkeywordscolor}{rgb}{0.6, 0.0, 0.9}
\definecolor{commentcolor}{rgb}{0.0, 0.6, 0.0}    % Green
\definecolor{BgColor}{gray}{0.93}
\definecolor{numbercolor}{rgb}{0.7, 0.7, 0.7}     %Gray
%\definecolor{numbercolor}{rgb}{0.85, 0.4, 0.0}     %aorange

% Define custom Python language
\lstdefinelanguage{Python}{
    keywords={def, return, if, elif, else, while, for, break, continue, pass, 
              import, from, as, class, try, except, finally, raise, with, 
              yield, assert, lambda, del, global, nonlocal, None, True, False, and, or, not, in, is},
    keywordstyle=\color{keywordcolor}\bfseries,
    ndkeywords={self, cls, __init__, __name__, __main__,print, object},
    ndkeywordstyle=\color{ndkeywordscolor},
    ...
    morestring=[s]{'''}{'''}, % Triple single quotes
    morestring=[s]{"""}{"""}, % Triple double quotes
...
\title{Analiza porównawcza metod poprawy jakości widzenia komputerowego dla głębokich sieci neuronowych na małym zbiorze danych} % INFO: tytuł pracy w języku polskim 
\titleShort{Metody poprawy jakości widzenia komputerowego}  % INFO: krótki tytuł pracy (do zamieszczenia w stopce, sklejony z imieniem i nazwiskiem autora nie powinien zająć więcej niż jedną linijkę)
%\titleEN{Smart Home automation system} % INFO: tytuł pracy w języku angielskim
\author{Maciej Danik}  % INFO: imię i nazwisko autora
\uczelnia{Politechnika Wrocławska} % INFO: nazwa uczelni
\wydzial{Wydział Informatyki i Telekomunikacji} % INFO: nazwa wydziału
\kierunek{Informatyczne Systemy Automatyki (ISA)} % IFO: nazwa kierunku
\specjalnosc{Inteligentne Systemy Przemysłu 4.0 (IPS)} % INFO: nazwa specjalności
\promotor{dr inż. Teodor Niżyński} % INFO: dane promotora 
\kvpl{Widzenie komputerowe} % INFO: słowa kluczowe po polsku
\kven{Computer vision} % INFO: słowa kluczowe po angielsku
\date{WROCŁAW, 2026} % INFO: miejscowość, rok złożenia pracy dyplomowej
...
\include{rozdzialy/rozdzial_01}
\include{rozdzialy/rozdzial_02}
\include{rozdzialy/rozdzial_03}
\include{rozdzialy/rozdzial_04}
\include{rozdzialy/rozdzial_05}
\include{rozdzialy/rozdzial_06}
\include{rozdzialy/dodatek_A}
