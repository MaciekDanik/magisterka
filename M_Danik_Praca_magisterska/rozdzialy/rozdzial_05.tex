...
W~niniejszym rozdziale przedstawiono wyniki uzyskane podczas treningu i~ewaluacji modeli detekcji w~przygotowanym środowisku eksperymentalnym. Analizę rozpoczęto od oceny modeli odniesienia, aby następnie zweryfikować i~porównać wpływ dwóch metod wzbogacania danych: iteracyjnego uczenia półnadzorowanego z~pseudoetykietowaniem (Metoda A) oraz syntetycznej augmentacji próbkami tła (Metoda B). Wszystkie zaprezentowane poniżej wartości liczbowe metryk (w~tym mAP oraz funkcje straty) wyznaczono na zbiorze walidacyjnym. Jeżeli dla danej konfiguracji istniało kilka uruchomień, do zestawień wykorzystano najdłuższy kompletny przebieg treningowy, a~krótkie uruchomienia kontrolne pominięto.
...
\caption{Wpływ metod wzbogacania danych na metryki detekcji modelu YOLOv8n na zbiorze walidacyjnym.}
...
\caption{Wpływ metod wzbogacania danych na metryki detekcji modelu YOLOv26n na zbiorze walidacyjnym.}
...
\caption{Wpływ metod wzbogacania danych na metryki detekcji modelu RT-DETR-L na zbiorze walidacyjnym.}
...
\section{Eksperyment B: Syntetyczna augmentacja próbkami tła (SSL Generative)}

W~drugim eksperymencie zbiór treningowy rozszerzono o~syntetycznie wygenerowane próbki negatywne (Metoda B). Do zbioru dodano obrazy przedstawiające różne typy tła bez obiektów docelowych. Rozwiązanie to miało zwiększyć różnorodność danych treningowych i~ograniczyć liczbę fałszywych detekcji związanych z~przeuczeniem modelu na konkretnych fragmentach tła.

\subsection{Analiza wyników dla modeli YOLO}

Dodanie syntetycznie wygenerowanych próbek negatywnych do zbioru treningowego nie poprawiło skuteczności modelu YOLOv8n względem modelu odniesienia (spadek mAP@0.5 o~1.4 p.p.). Precyzja wzrosła do 0.867, ale czułość obniżyła się do 0.724. Wyniki sugerują, że model stał się bardziej konserwatywny w~generowaniu detekcji, co ograniczyło część fałszywych wskazań, ale jednocześnie zwiększyło liczbę pominiętych obiektów.
...
FP                & 10               & 2                 & 9                  \\ \hline
FN                & 0                & 0                 & 0                  \\ \hline
Precyzja          & 0.29             & 0.67              & 0.31               \\ \hline
Czułość           & 1.00             & 1.00              & 1.00               \\ \hline
\end{tabular}
\end{table}

Analiza rysunku \ref{fig:baseline_comparison} oraz danych ilościowych z~tabeli \ref{tab:baseline_stats} pokazuje, że wybrany kafelek jest trudnym przypadkiem dla modeli odniesienia głównie ze względu na nadmiarowe wskazania. Wszystkie trzy modele poprawnie pokryły cztery obiekty referencyjne przy progu IoU równym $0.5$, jednak YOLOv8n oraz RT-DETR-L wygenerowały odpowiednio 10 i~9 fałszywie pozytywnych detekcji. Najbardziej precyzyjny w~tym przykładzie był model YOLOv26n.
...
FP                  & 3                & 2                 & 3                  \\ \hline
FN                  & 0                & 0                 & 0                  \\ \hline
Precyzja            & 0.57             & 0.67              & 0.57               \\ \hline
Czułość             & 1.00             & 1.00              & 1.00               \\ \hline
\end{tabular}
\end{table}


Zastosowanie iteracyjnego pseudoetykietowania zmniejszyło liczbę wskazań dla modeli YOLOv8n i~RT-DETR-L, co ograniczyło liczbę fałszywie pozytywnych detekcji na analizowanym kafelku. Wszystkie modele zachowały pełną czułość względem czterech obiektów referencyjnych, ale różniły się precyzją wynikającą z~liczby nadmiarowych ramek.
...
FP                      & 3                & 1                 & 13                 \\ \hline
FN                      & 0                & 0                 & 0                  \\ \hline
Precyzja                & 0.57             & 0.80              & 0.24               \\ \hline
Czułość                 & 1.00             & 1.00              & 1.00               \\ \hline
\end{tabular}
\end{table}

Dodanie syntetycznych obrazów tła (Metoda B) uwidoczniło różnice w~zachowaniu poszczególnych architektur. Dla modeli YOLO liczba wskazań pozostała niższa niż w~modelach odniesienia, co sugeruje działanie próbek negatywnych jako dodatkowego mechanizmu ograniczającego nadmiarowe detekcje. Największą precyzję w~tym przykładzie uzyskał YOLOv26n, który przy pełnej czułości wygenerował tylko jedną fałszywie pozytywną detekcję.

Odmienne zachowanie zaobserwowano dla modelu RT-DETR-L. Wprowadzenie syntetycznych danych zwiększyło liczbę fałszywie pozytywnych wskazań do 13, co jest spójne z~wynikami ilościowymi pokazującymi pogorszenie metryk tej architektury po zastosowaniu Metody B. Jedną z~możliwych przyczyn jest uczenie się zależności obecnych w~danych syntetycznych, które nie występowały w~danych rzeczywistych.
...
\begin{enumerate}
\item \textbf{YOLOv8n (model o~wysokim wyniku referencyjnym):} Architektura ta już w~modelu odniesienia osiągnęła wysoką wartość mAP@0.5. Obie metody wzbogacania danych obniżyły globalne metryki, choć w~analizie fałszywych detekcji widoczna była częściowa redukcja nadmiarowych wskazań. Wyniki sugerują, że dodatkowe dane nie poprawiły ogólnej jakości lokalizacji, ale mogły ograniczyć skłonność modelu do detekcji w~obszarach tła.
