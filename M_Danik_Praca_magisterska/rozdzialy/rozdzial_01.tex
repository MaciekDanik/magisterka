...
Głównym celem badawczym niniejszej pracy jest analiza wpływu metod wzbogacania danych treningowych na zdolność generalizacji modeli detekcji obiektów trenowanych w~warunkach ograniczonej dostępności danych. Cel ten odnosi się bezpośrednio do problemu wskazanego w~tytule pracy: poprawy jakości widzenia komputerowego dla głębokich sieci neuronowych uczonych na małym zbiorze danych.

W~ramach pracy sformułowano następujące cele szczegółowe:
\begin{enumerate}
    \item porównanie skuteczności wybranych architektur detekcyjnych YOLOv8n, YOLOv26n oraz RT-DETR-L w~zadaniu wykrywania worków z~piaskiem na ograniczonym zbiorze treningowym,
    \item ocena wpływu iteracyjnego uczenia półnadzorowanego (ang. \textit{Semi-Supervised Learning}, SSL) z~wykorzystaniem pseudoetykietowania na jakość detekcji i~stabilność procesu uczenia,
    \item ocena wpływu syntetycznej augmentacji danych, polegającej na generowaniu próbek negatywnych przy użyciu modeli dyfuzyjnych, na liczbę fałszywie pozytywnych detekcji,
    \item porównanie podatności analizowanych architektur na dodatkowe dane rzeczywiste i~syntetyczne,
    \item zaprojektowanie powtarzalnego środowiska eksperymentalnego umożliwiającego trenowanie, ewaluację oraz porównywanie modeli.
\end{enumerate}

Wyniki eksperymentalne analizowane w~rozdziale 5 odnoszą się do powyższych celów poprzez porównanie metryk mAP, precyzji, czułości, F1-score, przebiegu funkcji straty oraz liczby fałszywych detekcji. Pozwala to ocenić nie tylko końcową skuteczność modeli, lecz także stabilność uczenia i~podatność poszczególnych architektur na zastosowane metody wzbogacania danych.

Dodatkowym wyzwaniem implementacyjnym podjętym w~ramach pracy było zaprojektowanie i~wdrożenie zautomatyzowanego środowiska eksperymentalnego. Środowisko zostało oparte na technologiach konteneryzacji i~przystosowane do pracy na infrastrukturze obliczeniowej NVIDIA DGX A100, co umożliwiło realizację wieloetapowych eksperymentów obejmujących pętle uczenia Nauczyciel--Uczeń.
